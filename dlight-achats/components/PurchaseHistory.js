            ...
            <div key={p.id} className="bg-white rounded-xl border border-gray-200 p-3">
              <div className="flex justify-between items-start gap-2">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">{p.buyer_name} — {formatDate(p.created_at)}</div>
                ...
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  ...
                  {currentUser.role === "admin" && <button onClick={() => onDelete(p.id)} className="text-xs text-gray-400">supprimer</button>}
                </div>
              </div>
            </div>
          ))}
