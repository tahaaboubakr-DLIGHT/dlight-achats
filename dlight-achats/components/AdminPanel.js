    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
            ...
            <div key={u.id} className="flex justify-between items-center p-2.5 bg-gray-50 rounded-xl flex-wrap gap-1">
              <div>
                ...
                <div className="text-xs text-gray-500">
                  {u.email}{" "}
                  <span className={`uppercase text-[11px] ${u.role === "admin" ? "text-blue-600" : "text-gray-400"}`}>{u.role}</span>
                </div>
              </div>
              {u.role !== "admin" && (
                <button onClick={() => onRemoveUser(u.id)} className="text-xs text-red-500">Retirer</button>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col gap-2">
          <input type="email" value={ne} onChange={e => setNe(e.target.value)} placeholder="email@dlight.ma"
          ...
          <input type="text" value={nn} onChange={e => setNn(e.target.value)} placeholder="Prénom Nom"
            className="w-full h-11 px-3 rounded-xl border border-gray-300 text-[15px] focus:outline-none focus:ring-2 focus:ring-dlight/30" />
          <div className="flex gap-2">
            <select value={nr} onChange={e => setNr(e.target.value)}
              className="flex-1 h-11 rounded-xl border border-gray-300 px-3 text-[15px] bg-white">
              <option value="buyer">Acheteur</option>
              <option value="admin">Admin</option>
            </select>
            <button onClick={addU}
              className="h-11 px-5 bg-dlight text-white rounded-xl font-medium text-[15px] active:scale-95 transition">
              Ajouter
            </button>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        ...
        <div className="flex flex-col gap-2 mb-4">
          <input type="text" value={npn} onChange={e => setNpn(e.target.value)} placeholder="Nom du nouveau produit"
            className="w-full h-11 px-3 rounded-xl border border-gray-300 text-[15px] focus:outline-none focus:ring-2 focus:ring-dlight/30" />
          <div className="flex gap-2">
            <select value={npc} onChange={e => setNpc(e.target.value)}
              className="flex-1 h-11 rounded-xl border border-gray-300 px-3 text-sm bg-white">
          ...
          return (
            <div key={cat} className="mb-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {ps.map(p => (
                  <span key={p.id} className="inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-[13px]"
                    style={{ background: cc.bg, color: cc.text, border: `1px solid ${cc.border}` }}>
                    {p.name}
                    <button onClick={() => onRemoveProduct(p.id)}
                      className="text-[15px] leading-none opacity-60" style={{ color: cc.text }}>×</button>
                  </span>
                ))}
              </div>
            </div>
