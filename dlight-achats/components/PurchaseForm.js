...
export default function PurchaseForm({ products, onAdd, onNewProduct, lastPurchase, undoTimer, onUndo }) {
  ...
  async function handleSubmit() {
    ...
    if (!products.find(p => p.name.toLowerCase() === pn.trim().toLowerCase())) {
      await onNewProduct({ name: pn.trim(), category: c });
    }
    setPn(""); setCat(""); setQty(""); setPrice(""); setNote("");
    setSubmitting(false);
        ...
        <div className="bg-green-50 border border-green-300 rounded-xl px-3 py-2.5 flex justify-between items-center gap-2 animate-fade-in">
          <div className="flex-1 min-w-0">
            ...
            <div className="text-[13px] text-green-700 mt-0.5">
              {lastPurchase.product_name} — {lastPurchase.quantity} {lastPurchase.unit} × {formatDH(lastPurchase.unit_price)} = {formatDH(lastPurchase.total)}
            </div>
          </div>
          <button onClick={onUndo}
            className="bg-red-700 text-white rounded-lg px-3 py-2 text-[13px] font-medium whitespace-nowrap active:scale-95 transition">
            Annuler ({undoTimer}s)
          </button>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
          ...
          {cat ? (
            ...
            <div>
              ...
              <select value={cat} onChange={e => setCat(e.target.value)}
                className="w-full h-11 rounded-xl border border-gray-300 px-3 text-[15px] bg-white focus:outline-none focus:ring-2 focus:ring-dlight/30">
        ...
        <div className="grid grid-cols-[1fr_auto] gap-2.5 mb-3.5">
          ...
          <div>
            ...
            <select value={unit} onChange={e => setUnit(e.target.value)}
              className="h-11 rounded-xl border border-gray-300 px-2.5 text-[15px] bg-white focus:outline-none">
        ...
        <div className="mb-4">
          ...
          <input type="text" value={note} onChange={e => setNote(e.target.value)}
            placeholder="Ex: promotion, qualité..."
        ...
        <button onClick={handleSubmit} disabled={!ok || submitting}
          className={`w-full py-3.5 rounded-xl font-medium text-base text-white transition active:scale-[0.98] ${
            ok && !submitting ? "bg-dlight" : "bg-green-300 cursor-default"
          }`}>
