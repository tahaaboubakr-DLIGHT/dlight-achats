...
export default function PurchaseForm({ products, categories, units, onAdd, onNewProduct, lastPurchase, undoTimer, onUndo }) {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    const c = cat || "Non classe";
    const dateToUse = purchaseDate || today;
    await onAdd({ product: pn.trim(), category: c, qty: parseFloat(qty), unit, priceUnit: parseFloat(price), total: Math.round(total * 100) / 100, note: note.trim(), purchaseDate: dateToUse });
  ...
  return (
    <div className="flex flex-col gap-3">
      {lastPurchase && undoTimer > 0 && (
        <div className="bg-green-50 border border-green-300 rounded-xl px-3 py-2.5 flex justify-between items-center gap-2 animate-fade-in">
          <div className="flex-1 min-w-0">
            <div className="text-[13px] font-medium text-green-800">Dernier achat enregistre</div>
            <div className="text-[13px] text-green-700 mt-0.5">{lastPurchase.product_name} - {lastPurchase.quantity} {lastPurchase.unit} x {formatDH(lastPurchase.unit_price)} = {formatDH(lastPurchase.total)}</div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-5">
        ...
        <div className="mb-3.5">
          <label className="text-sm text-gray-500 block mb-1.5">Date d'achat <span className="text-gray-400">(aujourd'hui par defaut)</span></label>
        ...
        <div className="mb-4">
          ...
          <input type="text" value={note} onChange={e => setNote(e.target.value)} placeholder="Ex: promotion, qualite..." className="w-full h-11 px-3 rounded-xl border border-gray-300 text-[15px] focus:outline-none focus:ring-2 focus:ring-dlight/30" />
        </div>
