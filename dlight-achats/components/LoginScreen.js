    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="bg-white rounded-2xl border border-gray-200 p-8 max-w-[400px] w-full shadow-sm">
        ...
        <input type="email" value={email} onChange={e => { setEmail(e.target.value); setErr(""); }}
          onKeyDown={e => e.key === "Enter" && submit()} placeholder="prenom@dlight.ma"
          className="w-full h-11 px-3 rounded-xl border border-gray-300 text-base focus:outline-none focus:ring-2 focus:ring-dlight/30 focus:border-dlight" autoFocus />
        {err && <div className="mt-2 text-sm text-red-700 bg-red-50 rounded-lg px-3 py-2">{err}</div>}
        <button onClick={submit} className="w-full mt-4 bg-dlight text-white rounded-xl py-3 font-medium text-base active:scale-[0.98] transition">Se connecter</button>
      </div>
    </div>
  );
}
