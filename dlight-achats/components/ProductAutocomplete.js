...
(p.darija && p.darija.includes(value))
).slice(0, 15);
...
onFocus={handleFocus}
onKeyDown={onKey}
placeholder=“Cliquez ou tapez pour chercher…”
...
/>
{open && (
<div className="absolute top-[calc(100%+4px)] left-0 right-0 z-[200] bg-white border border-gray-300 rounded-xl max-h-[320px] overflow-y-auto shadow-lg">
{filtered.length === 0 ? (
<div className="px-3 py-4 text-center text-sm text-gray-400">
Aucun produit trouvé — tapez le nom pour en créer un nouveau
</div>
) : (
filtered.map((p, i) => {
...
className={`px-3 py-2.5 cursor-pointer flex justify-between items-center border-b border-gray-100 last:border-0 ${ i === hl ? "bg-gray-50" : "bg-white" }`}>
<div className="flex-1 min-w-0">
<div className="text-[15px] text-gray-900">{p.name}</div>
{p.darija && (
<div className="text-[13px] text-gray-400 mt-0.5" dir="rtl">{p.darija}</div>
)}
</div>
<span className=“text-[11px] px-2 py-0.5 rounded-full ml-2 flex-shrink-0”
