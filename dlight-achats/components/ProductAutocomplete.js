  ...
  return (
    <div ref={wrapRef} className="relative">
      <input
        ...
        onFocus={() => { if (value.length >= 2 && filtered.length) setOpen(true); }}
        onKeyDown={onKey}
          ...
          {filtered.map((p, i) => {
            ...
            return (
              <div key={p.id || p.name}
                ...
                className={`px-3 py-3 cursor-pointer flex justify-between items-center border-b border-gray-100 last:border-0 ${
                  i === hl ? "bg-gray-50" : "bg-white"
                }`}>
